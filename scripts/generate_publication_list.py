...
def main():
    ...
    parser.add_argument(
        "--include-preprints",
        action="store_true",
        help="Include preprints in the output",
    )
    ...
    pub_type_filter = ["journal", "conference"]
    if args.include_preprints:
        pub_type_filter.append("preprint")
    filtered_pubs = [pub for pub in publications if pub["type"] in pub_type_filter]
